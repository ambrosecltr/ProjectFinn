...
export function createCreativeManifest(options: CreativeManifestOptions): ToolsetManifest {
  ...
  if (options.image !== false) {
    commands.push({
      name: "image",
      description: "Create a new image from a prompt, or edit up to four referenced images. References can be JSON or selectors like file:file_1, path:/workspace/tmp/source.png, or url:https://example.com/source.png.",
      effects: ["write"],
      inputSchema: creativeImageInputSchema,
      argumentGuidance: [
        ...
        "numImages is 1-4. quality is low, medium, or high. outputFormat is jpeg, png, or webp.",
      ],
      examples: [
        { purpose: "Generate a new image", code: "await finn.creative.image({ prompt: \"editorial product photo of a matte black desk lamp on a walnut table\", imageSize: \"landscape_16_9\", numImages: 2, outputFormat: \"png\" })" },
        { purpose: "Edit one stored image", code: "await finn.creative.image({ prompt: \"make the background warmer and remove clutter\", images: \"file:file_123\", quality: \"high\", outputFormat: \"png\" })" },
  ...
  if (options.video !== false) {
    commands.push({
      name: "video",
      description: "Create a new video, animate one image, or edit one existing video with up to four reference images. Provide either image or video, not both.",
      effects: ["write"],
      inputSchema: creativeVideoInputSchema,
      argumentGuidance: [
        "prompt is required. Provide clear motion, scene, subject, and style instructions.",
        "Use image to animate an image, video to edit a video, or neither to generate from text. Do not provide both image and video.",
        "referenceImages accepts selector refs separated by | or a JSON array for edit guidance.",
