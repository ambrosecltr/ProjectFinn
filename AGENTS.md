...
- **Optional features**: Integrations toggle on when their API keys are present and selected. The web research provider is selected by `WEB_SEARCH_PROVIDER` (`auto`, `exa`, `parallel`, or `none`); speech, voice, image, and video providers are selected through `STT_PROVIDER`, `TTS_PROVIDER`, `IMAGE_PROVIDER`, and `VIDEO_PROVIDER` (`auto`, provider name, or `none`). Exa/Parallel web research, Fal/xAI creative media, Composio, STT, and TTS capabilities should not be exposed to LLMs unless configured and actually wired through the provider-neutral runtime clients.
