...
│   ├── media/                 # STT/TTS provider clients, file storage, attachment processing
│   ├── cron/                  # Shared scheduling helpers
│   ├── patterns/              # Pattern store + scheduler for scheduled/composio automations and run history
│   ├── integrations/          # External clients: Exa/Parallel web, Fal/xAI creative/media, Composio, BrowserUse, MCP manager
...
| External integrations | `packages/integrations/src/` | MCP, Exa/Parallel web, Fal/xAI creative/media, Composio, BrowserUse |
...
- **Patched dependencies**: `@spectrum-ts/imessage@6.1.1` is patched via `patches/@spectrum-ts%2Fimessage@6.1.1.patch`. The patch preserves iMessage captions by modeling caption + attachment payloads as Spectrum grouped content and stripping Apple's attachment placeholder characters. Remove this patch only after Spectrum upstream preserves text on iMessage attachment/group messages and the router caption/attachment regression tests pass without it.
...
- **Optional features**: Integrations toggle on when their API keys are present and selected. The web research provider is selected by `WEB_SEARCH_PROVIDER` (`auto`, `exa`, `parallel`, or `none`); speech, voice, image, and video providers are selected through `STT_PROVIDER`, `TTS_PROVIDER`, `IMAGE_PROVIDER`, and `VIDEO_PROVIDER` (`auto`, provider name, or `none`). Exa/Parallel web research, Fal/xAI creative media, Composio, STT, and TTS capabilities should not be exposed to LLMs unless configured and actually wired through the provider-neutral runtime clients. Creative worker schemas, examples, and runtime validation must be generated from selected provider capabilities: xAI image output is JPEG-only, while Fal image output supports JPEG, PNG, and WebP.
...
- **Project-local toolsets**: `packages/toolsets` is the home for Finn API/toolset definitions. Worker-facing families such as `files`, `web`, `creative`, `mcp`, `patterns`, `pattern`, and Puter toolsets are exposed through Secure Exec Finn JS workspace with generated searchable API docs, runtime requirements, and grant-filtered command surfaces. Hot-path file context uses the `files` APIs through Finn JS workspace with write-capable workspace access and document extraction suppressed. Keep toolsets project-wide, allowlisted, and gated by existing connector/runtime policy. Do not advertise or load a toolset unless the runtime is explicitly allowed to use it. For creative, preserve provider-specific capabilities through the integration router, runtime service, manifest schema, and executor validation so `workspace_search` never advertises unsupported arguments.
...
- **Models configurable per process** — `HOT_PATH_MODEL`, `WORKER_MODEL`, and `COMPACTOR_MODEL`. Format: `provider:model-name`; supported providers are `anthropic`, `openai`, `fireworks`, `deepseek`, and `openai-compatible`. Anthropic and OpenAI-compatible endpoints can use `DEFAULT_BASE_URL` or the process-specific `*_BASE_URL` override; OpenAI-compatible base URLs must include the API prefix, usually `/v1`. Defaults should be kept aligned with `.env.example` and README recommendations.
...
- **Complexity hotspots** (files >300 lines): `server/src/index.ts`, `integrations/src/mcp-manager.ts`, `integrations/src/fal.ts`, `integrations/src/xai.ts`, and `core/src/config.ts`.
